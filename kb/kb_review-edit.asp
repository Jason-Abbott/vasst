...
Set m_oReview = New kbReviewData
m_aData = m_oReview.GetArray()
If Trim(Request.Form("fldName")) <> "" Then	Call m_oReview.Save(m_aData)
m_bAdmin = CBool(GetSessionValue(g_USER_TYPE) = CStr(g_USER_ADMIN))
%>
<html>
<title><%=g_sORG_NAME%>: Submit your review</title>
<head>
...
<center>
<form name='<%=m_sFORM_NAME%>' action='kb_review-edit.asp?id=<%=m_aData(m_REVIEW_ID)%>' method='post' onSubmit="return IsValid('<%=m_sFORM_NAME%>',m_oFields);">
<% Call m_oLayout.WriteTitleBoxTop("Review Submission", "", "") %>
<table cellspacing='0' cellpadding='0' border='0'>
<tr>
	<td class='Required'>Review Name:</td>
	<td class='FormInput' valign='top'><input type='text' name='fldName' size='30' maxlength='75' value='<%=m_aData(m_REVIEW_NAME)%>'></td>
<tr><td></td><td class='FormNote'>maximum of 75 characters</td>
<tr>
	<td class='Required'>URL:</td>
	<td class='FormInput' valign='top'><input type='text' name='fldURL' maxlength='75' size='30' value='<%=m_aData(m_REVIEW_URL)%>'></td>
...
<tr>
	...
	<td class='FormInput' valign='top'>
		<% Set m_oUser = New kbUser : Call m_oUser.WriteUserList("fldAuthor", m_aData(m_REVIEW_AUTHOR_ID), g_STATUS_APPROVED, "not listed") %>
		<% if m_bAdmin then %>
...
<tr>
	...
	<td class='FormInput' valign='top'><textarea rows='6' cols='45' name='fldDescription'><%=m_aData(m_REVIEW_TEXT)%></textarea></td>
...
<tr>
	...
	<tr>
		...
		<% Call m_oLayout.WriteCategoryList("fldCategories", m_aData(m_REVIEW_CATS), 6, g_ITEM_REVIEW) %>
		</td>
		<td valign='top' class='FormNote'><br><%=g_sMSG_MULTI_SELECT%></td>
	<tr>
		<td align='right' valign='bottom'>
		<% Call m_oLayout.WriteToggleImage("btn_save", "", "Save Review", "class='Image'", true) %>
		</td>
	</table>
	</td>
</table>
<% Call m_oLayout.WriteBoxBottom("") %>
<% Set m_oLayout = Nothing : Set m_oReview = Nothing %>
