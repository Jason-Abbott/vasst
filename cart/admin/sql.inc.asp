...
'				On Error Resume Next
		openDB
		cartDB.CommandTimeout = 0
		If (Len(runSQL) > 0) Then
			SQLConsoledbRS.Open runSQL, cartDB, 2, 3
			If (Err <> 0) Then
	...
	'	perRow = 5
		...
		Set dbTest = cartDB.Execute("SELECT * FROM MSysObjects")
		If (dbTest.EOF) Then
			dbType = "SQL"
			Set tableList = cartDB.Execute("SELECT sysobjects.name, sysusers.name as owner FROM sysobjects, sysusers WHERE type = 'U' AND sysobjects.uid = sysusers.uid ORDER BY sysobjects.name")
		Else
			dbType = "Access"
			Set tableList = cartDB.Execute("SELECT name FROM MSysObjects WHERE type = 1 AND name NOT LIKE 'MSys%' ORDER BY name")
		End If
...
'						dataScript = dataScript & ("var " & tableList("owner") & " = new Object();" & vbNewline)
				If (dbType = "SQL") Then
					SQLConsoledbRS.Open "SELECT count(*) as recordCount FROM " & tableList("owner") & "." & tableList("name") & "", cartDB, 2, 3
				Else
					SQLConsoledbRS.Open "SELECT count(*) as recordCount FROM " & tableList("name") & "", cartDB, 2, 3
				End If
				tblRecordCount = SQLConsoledbRS("recordCount")
				SQLConsoledbRS.Close
				If (dbType = "SQL") Then
					SQLConsoledbRS.Open "SELECT TOP 0 * FROM " & tableList("owner") & "." & tableList("name") & "", cartDB, 2, 3
				...
				Else
					SQLConsoledbRS.Open "SELECT * FROM " & tableList("name") & "", cartDB, 2, 3
